...
namespace m3_zapletal.Eshop.Areas.Customer.Controllers
{
    [Area("Customer")]
        ...
        public double AddOrderItemsToSession(int? productId)
        {
            ...
            if (product != null)
            {
                OrderItem orderItem = new OrderItem()
                {
                    ...
                    Product = product,
                    Amount = 1,
                    Price = product.productPrice   //zde pozor na datový typ -> pokud máte Price v obou případech double nebo decimal, tak je to OK. Mě se bohužel povedlo mít to jednou jako decimal a jednou jako double. Nejlepší je datový typ změnit v databázi/třídě, tak to prosím udělejte.
                };
                ...
                if (HttpContext.Session.IsAvailable)
                {
                    ...
                    if (orderItemInSession != null)
                    {
                        ...
                        orderItemInSession.Price += orderItem.Product.productPrice;   //zde pozor na datový typ -> pokud máte Price v obou případech double nebo decimal, tak je to OK. Mě se bohužel povedlo mít to jednou jako decimal a jednou jako double. Nejlepší je datový typ změnit v databázi/třídě, tak to prosím udělejte.
                    }
                    else
                    {
                    ...
                    totalPrice += orderItem.Product.productPrice;
